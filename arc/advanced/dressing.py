...
class DressedInteractions(PairStateInteractions):
    ...
    def __init__(self, atom, n, l, j,
        ...
        Args:
            ...
            n (int): principal quantum number for the dressed you are coupling to.
            l (int): orbital angular momentum for the dressed you are coupling to.
            j (float): total angular momentum for the dressed you are coupling to.
            s (float): optional, spin state of the atoms. Default value
                ...
                Example:
                    ...
                    from :math:`F=4, m_F=4` ground state with circular polarization
                    :math:`\sigma_+`.
    ...
    def defineBasis(self,theta,phi, dn,dl, deltaMax, Bz = 0,progressOutput=False,debugOutput=False):
        ...
        is True while diagonalizing to give access to all pair states independent
        of the reference state chosen.

        """
        super().defineBasis(theta,phi, dn,dl, deltaMax, Bz = Bz,progressOutput=progressOutput,forceNolimitMj=True,debugOutput=debugOutput)

    def diagonalize(self,rangeR,noOfEigenvectors,eigenstateDetuning = 0.,
                         sortEigenvectors = False,progressOutput = False,
                         debugOutput = False):
        """Finds eigenstates in the atom pair basis. Overloads the PairStateInteractions
            method.

        """

        super().diagonalize(rangeR,noOfEigenvectors,drivingFromState = [0,0,0,0,0],
                             eigenstateDetuning = eigenstateDetuning,sortEigenvectors = sortEigenvectors,
                             progressOutput = progressOutput,debugOutput = debugOutput)
    ...
    def defineDressing(self,laserDetuning,laserPolarization,groundState,
        ...
        Args:
            ...
            laserPolarization (int): specifies transition that the driving field couples to,
        ...
        self.groundStateEnergy = self.atom1.getEnergy(groundState[0][0],groundState[0][1],
                                groundState[0][2])*C_e + groundStateOffset*C_h # Joules
        ...
        self.laserFreq = (self.atom1.getEnergy(self.n,self.l,self.j)*C_e-\
                                    self.groundStateEnergy + laserDetuning*C_h)/hbar # Joules/hbar
        ...
        for state in self.basisStates:
            for mI1 in self.mIlist:
                for mI2 in self.mIlist:
                    self.extendedBasisStates.append(np.append(state,[mI1,mI2]))
        ...
        #couples to an excited state to not divide by 0 later
            ...
            if np.abs(groundState[0][3]+laserPolarization) - 0.1 > self.j: # verify mj < J for first ground state
                self.dmeGSReference = 0
            else:

                self.dmeGSReference = self.atom1.getDipoleMatrixElement(groundState[0][0],groundState[0][1],\
                            groundState[0][2],groundState[0][3],self.n,self.l,self.j,groundState[0][3]+\
            ...
            if len(groundState)>1:
                for gs in groundState[1:]:
                    if np.abs(gs[3]+laserPolarization) - 0.1 > self.j: # verify mj < J for second ground state
                        dmeGS = 0
                    else:
    ...
    def calculateDressedPotential(self,debugOutput=False):
        ...
        for basisStateIndex, basisState in enumerate(self.extendedBasisStates):
            ...
            n1 = int(n1); l1 = int(l1); n2 = int(n2); l2 = int(l2)

            energyState1 = self.atom1.getEnergy(n1,l1,j1)*C_e+self.atom1.getZeemanEnergyShift(l1,j1,mj1,self.Bz) # Joules
            energyState2 = self.atom1.getEnergy(n2,l2,j2)*C_e+self.atom1.getZeemanEnergyShift(l2,j2,mj2,self.Bz) # Joules
        ...
        self.eigValsExtended = np.repeat(self.eigVals,self.mIMultiplicity,axis=1)
        twoPhotonDetunings = (2*self.laserDetuning-self.eigValsExtended*1e9)*2*np.pi#rad/s
        #finally calculate the dressed potential as a sum of \Omega_pair^2/\Delta_2 photon
    ...
    def _addStateToRabiFreqDict(self,state, debugOutput=False):
        ...
        Args:
            state ([int,int,float,float,float]): State we couple to from self.groundState.
            Last member of state is the nuclear spin quantum number mI.
        """
        ...
        if not (state in self.rabiFreqDict):
            for idx, gs in enumerate(self.groundState):
                if (self.groundStateCoeffs[idx] > 0) and (state[3] == gs[3]+self.laserPolarization) \
                           and (state[4]==gs[4]):
                    #calculate Rabi frequency coupling from the ground state gs to state
    ...
    def plotLevelDiagram(self,  highlightColor='red',
        ...
        """Plots pair state level diagram. In case the dressed potential has been calculated,
        highlits the states based on the pair Rabi frequency coupling from the ground state
        ...
        #check whether the dressing calculation has been performed
        #if yes than use the pair Rabi frequencies as a highlight
        if hasattr(self,'eigValsExtended'):
    ...
    def plotDressedPotential(self,includePertPrediction=False,color=None):
        ...
        Args:
            includePertPrediction (боол): optional, False by default. Calculate and plot the perturbative prediction for the dressed potential.
            color (string): Color of the dressing potential line.
        """
        ...
        if includePertPrediction:
            if self.measuredRabiFreq:
                ...
                for idx,gs in enumerate(self.groundState):
                    rf+=self.groundStateCoeffs[idx]*self.atom1.getRabiFrequency(gs[0],gs[1],\
                        gs[2],gs[3],self.n,self.l,self.j,self.laserPolarization,self.laserPower,\
                        self.laserWaist)
            ...
            self.ax_d.axhline(pertPrediction,0,10,color='gray',linewidth=2.5,zorder=1)

        self.ax_d.scatter(self.r,self.dressedPotential/(2*np.pi*1e3),color=color)
